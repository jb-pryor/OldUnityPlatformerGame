...
public class ColorSwap_HeroKnight : MonoBehaviour
{
    ...
    public void InitColorSwapTex()
    {
        ...
        Debug.Log("Swap texture assigned to material.");
    ...
    {
        Debug.Log($"Setting index {index} to color {color}");
    ...
    public void SwapColorsFromSet(Color[] newColors)
    {
        if (m_spriteRenderer == null)
        {
            Debug.LogError("SpriteRenderer component is missing on the player.");
            return;
        }
        
        Debug.Log("Attempting to swap colors...");
        Debug.Log("New colors: " + string.Join(", ", newColors));
        Debug.Log($"Swapping colors: {newColors.Length} new colors, {m_sourceColors.Length} source colors");
        for (int i = 0; i < m_sourceColors.Length && i < newColors.Length; i++)
        {
            Debug.Log($"Source color {m_sourceColors[i]} gives index {(int)(m_sourceColors[i].r * 255.0f)}");
            Debug.Log($"Swapping source {m_sourceColors[i]} to {newColors[i]}");
            SwapColor((int)(m_sourceColors[i].r * 255.0f), newColors[i]);
        }
        m_colorSwapTex.Apply();
        Debug.Log("Color swap applied.");
    }

    void Start()
    {
        Color[] testColors = { Color.red, Color.green, Color.blue };
        SwapColorsFromSet(testColors);
    }
}